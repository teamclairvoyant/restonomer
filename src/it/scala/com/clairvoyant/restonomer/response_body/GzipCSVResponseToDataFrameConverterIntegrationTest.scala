...
class GzipCSVResponseToDataFrameConverterIntegrationTest
  ...
  it should "convert the gzip CSV file response body into a dataframe" in {
    runCheckpoint(checkpointFileName = "checkpoint_gzip_csv_response_dataframe_converter.conf")
    outputDF should matchExpectedDataFrame("expected_gzip_csv_response.json")
  }
