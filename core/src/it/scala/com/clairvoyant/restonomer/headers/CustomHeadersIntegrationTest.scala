...
class CustomHeadersIntegrationTest extends IntegrationTestDependencies {
  ...
  it should "add no custom headers when empty headers provided" in {
    restonomerContext.runCheckpoint(checkpointFilePath = s"$mappingsDirectory/checkpoint_add_empty_custom_headers.conf")
  }

  it should "add no custom headers when headers are not provided" in {
    restonomerContext.runCheckpoint(checkpointFilePath = s"$mappingsDirectory/checkpoint_blank_custom_headers.conf")
  }

  it should "add custom headers when headers are provided" in {
    restonomerContext.runCheckpoint(checkpointFilePath = s"$mappingsDirectory/checkpoint_add_custom_headers.conf")
  }
