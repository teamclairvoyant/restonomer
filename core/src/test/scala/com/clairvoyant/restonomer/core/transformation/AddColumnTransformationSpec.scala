...
class AddColumnTransformationSpec extends CoreSpec {
  ...
  "transform() - with column-data-type" should "transform the dataframe" in {
    ...
    actualRestonomerResponseTransformedDF should matchExpectedDataFrameApplyingSanitised(
      expected = expected,
      expectedSchemaTransformer = DataFrameSchemaTransformer.default
    )
  }
